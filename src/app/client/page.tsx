                ...
                <div key={payment.id} className="card">
                  ...
                  {payment.status === 'OPEN' || payment.status === 'OVERDUE' ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <a 
                        href={`upi://pay?pa=${payment.upiId}&pn=${encodeURIComponent("Payment")}&tr=TXN${Date.now()}&am=${payment.amount.toFixed(2)}&cu=INR&tn=FeePayment`}
                        className="btn"
                      ...
                      <details style={{ width: '100%' }}>
                        ...
                        <div style={{ marginTop: '8px', padding: '16px', backgroundColor: '#fff', borderRadius: '12px', textAlign: 'center' }}>
                          <h4 style={{ color: '#000', fontWeight: 'bold', marginBottom: '12px' }}>Scan to Pay via UPI</h4>
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img 
                            src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(`upi://pay?pa=${payment.upiId}&pn=Payment&tr=TXN${Date.now()}&am=${payment.amount.toFixed(2)}&cu=INR&tn=FeePayment`)}`}
                            alt="UPI QR Code"
